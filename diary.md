...
fifo2adc和spi2fifo的debu完成，所有的部分都debug完成了，后面可以合在一起了
还有一个问题，就是百兆以太网尚未完成，后面可能还得多处理一下了

004C///////////////////////////////////////////////////

写入寄存器完成，顺便改了一下cs_reg的写入，之前的对ZCheck了解的比较少，现在多了新的了解
所以把之前的都改了，现在可以做最后的ADC部分了, 还需要将之前的错误逻辑改一下

004B///////////////////////////////////////////////////

完成了芯片的初始化的测试工作，之后开始写入寄存器测试

004A///////////////////////////////////////////////////

完成了寄存器写的问题，之后开始初始化测试

0049///////////////////////////////////////////////////

按照Intan公司的意思，LVDS电平标准先不去做了，使用标准电平debug现在看来问题不大，成功读取寄存器了
之后开始测试写入寄存器

0048///////////////////////////////////////////////////

时钟clk根本不能用，忘记了相同的bank只能用一样的电压标准，之前的设计圈出问题了
之后得全部重新改，现在只能先测试其他的东西了
首先测试百兆以太网，问题很大，现在还无法完成
先不纠结了，打算先开始做adc部分的debug

0047///////////////////////////////////////////////////

测试板第二步，现在确定时钟没有问题，后面测试测试板时钟clk

0046///////////////////////////////////////////////////

测试板测试第一步，现在确定leb部分没有问题，后面开始验证核心板时钟是否正常工作
...
cs_cmd写完了最后一个问题，现在问题解决了，感觉cs部分全部弄完了，后面可以全力弄intan的时序了

0043///////////////////////////////////////////////////

cs_cmd还是需要在改一点，但是大体的框架应该解决了，cs_cmd基本弄完了
...
处理完基于fifo的仿真mac部分，后面直接开始编写cs_cmd部分的仿真，需要留有mac中间cmd_make的接口

003F///////////////////////////////////////////////////

改了一点intant的部分，感觉这样可以更加仿真之前的时序，后面可以先把cs_cmd的部分弄完了

003E///////////////////////////////////////////////////

重写了fifo2adc的时序，之前的有点改不回来了，而且把fifoi的特性都改变了，改成了FWFT Fifo了
这样的时序能简单一些，但是FWFT和Standard模式的FIFO有什么区别现在还不知道，之后如果因为这里不行再改吧
现在仿真弄完了应该没问题了，后面就开始弄时序约束了
